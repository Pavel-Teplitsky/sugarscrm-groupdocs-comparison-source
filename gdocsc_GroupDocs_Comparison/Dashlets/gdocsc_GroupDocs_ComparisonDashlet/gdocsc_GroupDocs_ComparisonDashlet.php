...
class gdocsc_GroupDocs_ComparisonDashlet extends DashletGeneric { 
     function gdocsc_GroupDocs_ComparisonDashlet($id, $def = null) {
        ...
        //$this->columns = $dashletData['gdocsc_GroupDocs_SignatureDashlet']['columns'];
    ...
    function display() {
		...
		while (($row = $GLOBALS['db']->fetchByAssoc($result)) != null) {
			$iframe.= '<iframe id="groupdocs_comparison_iframe" height="400" frameborder="0" width="100%" src="https://apps.groupdocs.com/document-comparison/embed/'.$row['embed_key'].'/'.$row['file_id'].'?&referer=' . $cmsName . '/' . $cmsVersion . '"></iframe><br>';
		}
		return $iframe;
    }
